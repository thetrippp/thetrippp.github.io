...
for cat_name, feeds in categories.items():
    ...
    Data: {cat_news_text}
    """
    editor_response = client.models.generate_content(model='gemini-1.5-flash', contents=editor_prompt).text
    ...
    for story in top_stories:
        ...
        """
        story_coverage = client.models.generate_content(model='gemini-1.5-flash', contents=journalist_prompt).text
    ...
    """
    weekly_recap = client.models.generate_content(model='gemini-1.5-flash', contents=recap_prompt).text
    time.sleep(5) # Strict 5-second sleep to respect the 15 RPM limit
